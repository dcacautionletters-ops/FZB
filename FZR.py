...
if uploaded_file:
    ...
    if st.button("Generate Reports", use_container_width=True):
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            if sheets_created == 0:
                pd.DataFrame({"Message": ["No data matched your criteria. Check Batch Names."]}).to_excel(writer, sheet_name="No Data Found")

        st.success("Process Complete!")
        st.download_button("📥 Download Excel Report", output.getvalue(), "VMS_Report.xlsx", use_container_width=True)
